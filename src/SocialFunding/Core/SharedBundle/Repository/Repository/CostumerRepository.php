...
class CostumerRepository extends EntityRepository{
    ...
    public function findAllOngsDonated($costumer)
    {
        return $this->getEntityManager()
            ->createQueryBuilder()
            ->select('o, c')
            ->from('SocialFunding\Core\SharedBundle\Entity\Order', 'o')
            ->leftJoin('o.campaign', 'c')
            ->where('o.costumer = :costumer')
            ->setParameters(array('costumer' => $costumer))
            ->getQuery()
            ->getResult();
    }
